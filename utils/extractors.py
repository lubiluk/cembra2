...
class StackCNN(BaseFeaturesExtractor):
    def __init__(self,
                 observation_space: gym.spaces.Box,
                 linear_dim: int,
                 features_dim: int = 256,
                 n_channels: int = 1):
        super(StackCNN, self).__init__(observation_space,
                                        features_dim + linear_dim)
        ...
        # We assume CxHxW images (channels first)
        # Re-ordering will be done by pre-preprocessing or wrapper
        self.n_stack = int(observation_space.shape[0] / (n_channels + 1))

        self.cnn = nn.Sequential(
            nn.Conv2d(self.n_channels * self.n_stack, 4, kernel_size=8, stride=4, padding=0),
            nn.ReLU(),
    ...
    def _get_camera_obs(self, obs):
        idxs = [i * (self.n_channels + 1) + j for j in range(self.n_channels) for i in range(self.n_stack)]
        return obs[:, idxs, :, :]

    def _get_linear_obs(self, obs):
        idxs = [i * (self.n_channels + 1) + j + 1 for j in range(self.n_channels) for i in range(self.n_stack)]
        return obs[:, idxs, :self.linear_dim, 0]