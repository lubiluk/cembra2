...
class ReplayBuffer:
    ...
    """
    def __init__(self, obs_dim, act_dim, goal_dim, size, device=None):
        ...
        self.act_buf = torch.zeros(core.combined_shape(size, act_dim),
                                   dtype=np.float32,
                                   device=device)
        self.rew_buf = torch.zeros(size, dtype=np.float32, device=device)
        self.done_buf = torch.zeros(size, dtype=np.float32, device=device)
        self.dgoal_buf = torch.zeros(core.combined_shape(size, goal_dim),
                                     dtype=np.float32,
                                     device=device)
        self.agoal_buf = torch.zeros(core.combined_shape(size, goal_dim),
                                     dtype=np.float32,
                                     device=device)
    ...
    def store(self, obs, act, rew, next_obs, done, dgoal, agoal, info):
        ...
        self.dgoal_buf[self.ptr] = torch.from_numpy(dgoal)
        self.agoal_buf[self.ptr] = torch.from_numpy(agoal)
...
def sac_her_cnn(env_fn,
    ...
    def synthesize_experience(env,
        ...
        for idx in range(ep_end):
            obs = ep['obs'][idx].numpy()
            act = ep['act'][idx].numpy()
            obs2 = ep['obs2'][idx].numpy()
            agoal = ep['agoal'][idx].numpy()
            info = ep['info'][idx]
            ...
            for _ in range(num):
                ...
                replay_buffer.store(obs, act, rew, obs2, False, sel_agoal,
                                    agoal, info)
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            for j in range(update_every):
                ...
                og_batch = dict(
                    obs=torch.concatenate([batch['obs'], batch['dgoal']],
                                          dim=-1),
                    obs2=torch.concatenate([batch['obs2'], batch['dgoal']],
                                           dim=-1),
                    act=batch['act'],
                    rew=batch['rew'],
                    done=batch['done'])
...
if __name__ == '__main__':
    ...
    sac_her(lambda: gym.make(args.env),
            actor_critic=core.MLPActorCritic,
