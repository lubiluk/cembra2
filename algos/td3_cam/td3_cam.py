...
class ReplayBuffer:
    ...
    def _get_batch(self, idxs):
        obs = [{
            "camera_bottom": c,
            "joints_state": j
        } for c, j in zip(self.obs_img_buf[idxs], self.obs_lin_buf[idxs])]
        obs2 = [{
            "camera_bottom": c,
            "joints_state": j
        } for c, j in zip(self.obs2_img_buf[idxs], self.obs2_lin_buf[idxs])]
...
if __name__ == '__main__':
    ...
    td3_cam(lambda: gym.make(args.env),
                actor_critic=core.MLPActorCritic,
